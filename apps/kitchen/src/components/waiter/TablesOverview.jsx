...
export default function TablesOverview({ onSelectTable }) {
  ...
  const fetchTablesAndOrders = async () => {
    ...
    if (tablesData && ordersData) {
      ...
      sessionStorage.setItem('chef_tables', JSON.stringify(computedTables));
    }
  };
  ...
  return (
    <div className="p-4 max-w-md mx-auto pb-24 pt-6 bg-theme-bg min-h-[100dvh]">
      <div className="mb-6 px-2">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-black tracking-tight italic text-theme-text-main">tablenet</h1>
            <span className="bg-theme-primary/10 text-theme-primary px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-widest">waiter (kitchen)</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={toggleSound}
